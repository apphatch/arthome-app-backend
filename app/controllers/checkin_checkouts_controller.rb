class CheckinCheckoutsController < ApplicationController
  def index
    ...
    if @current_app.name == 'osa-webportal'
      @records = @records.where("created_at >= ?", DateTime.parse(params[:date_from])) unless params[:date_from].empty?
      @records = @records.where("created_at <= ?", DateTime.parse(params[:date_to])) unless params[:date_to].empty?
