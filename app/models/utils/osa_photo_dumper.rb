  ...
  class OsaPhotoDumper
    ...
    def self.dump_to_disk photos
      ...
      photos.find_each do |photo|
        ...
        #time/shop structure
        Dir.mkdir(File.join(root, time_repr)) unless Dir.exist?(File.join(root, time_repr))
        Dir.mkdir(File.join(root, time_repr, shop_repr)) unless Dir.exist?(File.join(root, time_repr, shop_repr))
