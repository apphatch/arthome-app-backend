module Filterable::ByDate
  ...
  def updated_at_filter obj, params
    if params[:date_from].present? && params[:date_to].present? && obj.respond_to?(:updated_at)
      return obj.updated_at.between?(params[:date_from], params[:date_to])
    end
    return true
  end
